#!/usr/bin/env bash
# HNSCC — same entrypoint pattern as run_orion_all.sh.
#
#   MODE=prepare bash scripts/run_hnscc_all.sh
#   MODEL=pix2pix MODE=train bash scripts/run_hnscc_all.sh
#   MODEL=vanilla_fm MODE=train bash scripts/run_hnscc_all.sh  # cross-attn via env profile
#
set -euo pipefail
...
echo "===== HNSCC  MODEL=${MODEL}  MODE=${MODE}  DATAROOT=${DATAROOT} ====="
...
if is_native; then
  ...
  bash "${ROOT}/scripts/run_hnscc_native.sh"
else
