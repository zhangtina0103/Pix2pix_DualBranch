#!/usr/bin/env bash
# HNSCC: train.py → test.py → post_process_hnscc.py (joint 4ch mIF panel).
set -euo pipefail
...
MODEL="${MODEL:-pix2pix}"
MODE="${MODE:-all}"
HNSCC_SRC="${HNSCC_SRC:-./data/hnscc}"
DATAROOT="${DATAROOT:-./datasets/hnscc}"
GPU_IDS="${GPU_IDS:-0}"
HNSCC_OUTPUT_NC="${HNSCC_OUTPUT_NC:-4}"

# shellcheck source=/dev/null
source "${ROOT}/scripts/hnscc_scratch_env.sh"
source "${ROOT}/scripts/hnscc_model_profiles.sh"
...
prepare() {
  echo "==> prepare HNSCC → ${DATAROOT}"
  local prep_args=(--dst "${DATAROOT}" --mode "${HNSCC_MODE:-cross_modal}" --tile-size "${HNSCC_TILE_SIZE:-512}")
  if [[ -f "${HNSCC_ZIP:-}" ]]; then
    prep_args+=(--zip "${HNSCC_ZIP}")
  else
    prep_args+=(--src "${HNSCC_SRC}")
  fi
  python scripts/prepare_hnscc.py "${prep_args[@]}"
  source "${ROOT}/scripts/hnscc_model_profiles.sh"
  echo "test tiles for eval: NUM_TEST=${NUM_TEST} output_nc=${HNSCC_OUTPUT_NC}"
}

train() {
  local netg_label="${NETG:-(none)}"
  assert_py_model
  echo "==> [hnscc] MODEL=${MODEL} PY_MODEL=${PY_MODEL} netG=${netg_label} name=${TRAIN_NAME}"
  echo "    dataroot=${DATAROOT} output_nc=${HNSCC_OUTPUT_NC} ${LOAD_SIZE:-512}→${CROP_SIZE:-512}"
  ...
  case "${PY_MODEL}" in
    ...
    vanilla_fm)
      extra+=(
        ...
        --fm_channel_weights "${FM_CHANNEL_WEIGHTS:-1,1,1,1}"
      )
  ...
  local train_args=(
    ...
    --gpu_ids "${GPU_IDS}"
    --input_nc 3 --output_nc "${HNSCC_OUTPUT_NC}"
...
test_one() {
  ...
  assert_py_model
  echo "==> [hnscc test] name=${name} epoch=${epoch} num_test=${num_test}"
  local extra=()
  [[ -n "${DATASET_MODE:-}" ]] && extra+=(--dataset_mode "${DATASET_MODE}")
  if [[ "${PY_MODEL}" == "vanilla_fm" ]]; then
    extra+=(
...
metrics_one() {
  ...
  python scripts/post_process_hnscc.py --srcdir "results/${name}/test_${epoch}/"
}
